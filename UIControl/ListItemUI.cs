...
namespace UIControl_MonoGame.UIControl
{
    public class ListItemUI : Cordinator, IControlUI, IToXml
    {
        ...
        public Сolumn[] Сolumns { get => [.. _сolumns]; }
        ...
        public void ControlEvent(MouseState getMouse, KeyboardState getKey, uint getJoy = 1)
        {
            ...
            else if (Focused & getKey.IsKeyDown(Keys.Down) & Cliker == false)
            {
                Cliker = true;
                SelectedRow++;
                if (SelectedRow + StartItem == DataItems.Count) { SelectedRow--; }
                else if (Item_end) { StartItem++; SelectedRow--; }
            ...
            else if (Focused & getKey.IsKeyDown(Keys.Up) & Cliker == false)
            {
                Cliker = true;
                SelectedRow--;
                if (SelectedRow + StartItem == -1) SelectedRow = 0;
                else if (SelectedRow <= 0 & StartItem!=0) { 
                     StartItem--;
                    SelectedRow = 0;
        ...
        public void Draw(GameTime gameTime, SpriteBatch spriteBatch)
        {
            ...
            Color sel;
            int itemNow = 0;
            for (int t = StartItem; t < DataItems.Count; t++)
            {
                var row = DataItems[t];
                int first = RectObjectUI.X;
                if (SelectedRow == itemNow) sel = SelectionColor;
                ...
                itemNow++;
                if (start > (RectObjectUI.Height + RectObjectUI.Y - row.Column[0].HeightRows + Thickness)) // the rows went beyond the control
                {
                    if (SelectedRow+ StartItem == t) Item_end = true;
                    else Item_end = false;
                    break;
                }
            }
        ...
        /// <summary>
        /// By default, the row for the listitem
        /// </summary>
        public class StandartItem(string str)
        {
            public string Text { get; set; } = str;
        }
        ...
        /// Row
        /// </summary>
        /// <param name="obj">Reference to the data binding object</param>
        /// <param name="сolumn">Column Parameters</param>
